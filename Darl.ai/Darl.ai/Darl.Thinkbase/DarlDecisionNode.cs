/// <summary>
/// DarlDecisionNode.cs - Core module for the Darl.dev project.
/// </summary>
...
namespace Darl.Thinkbase
{
    ...
    public class DarlDecisionNode
    {
        ...
        /// <param name="currentOutputsAsInputs">The list of available outputs as inputs not yet used.</param>
        ...
        internal void GenerateNodes(IODefinitionNode? currentInput, OutputDefinitionNode currentOutput, List<IODefinitionNode> currentInputs, List<int> currentIndices, int currentIndex, int depth)
        {
            ...
            if (!inputs.Any() || indices.Count < minDataPoints || double.IsNaN(info))
            {
                // its a leaf node
            ...
            IODefinitionNode? bestInput = null;
            double bestInfoGain = 0.0;
            foreach (var trialInput in inputs)
            {
                double split = 0.0;
            ...
            if (bestInput == null || bestInfoGain < (leafInfoThreshold * (depth - 1)))
            {
                if (input == null)
                {
                    throw new MetaRuleException("Insufficient information in the data set to learn anything.");
                }
                outputPartitions = output.ChoosePartitions(indices);
            ...
            newInputs = new List<IODefinitionNode>();
            foreach (var currInput in inputs)
            {
            ...
            if (level != 0) // !root node
            {
                ...
                if (merged)
                {
                    ...
                    foreach (int candidate in mergedIndexes)
                    {
                        ...
                        input.WriteTerm(ref orTerms, input.categories[candidate]);
                        if (nCount > 1)
                        {
                    ...
                    input.WriteTerm(ref middle, input.categories[index]);
                level++;
            }
            if (IsLeaf())
            { // leaf node, 
        ...
        protected int CompleteRules(ref string start, ref string middle)
        {
            ...
            foreach (int partition in outputPartitions.Keys)
            {
                ...
                if (dRelevance >= 1.0 / output.categories.Count && dRelevance >= minimumConfidence && indices.Count >= minimumSupport) //added for assoc rule apps
                {
