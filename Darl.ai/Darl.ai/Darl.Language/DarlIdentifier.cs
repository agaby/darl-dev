/// <summary>
/// DarlIdentifier.cs - Core module for the Darl.dev project.
/// </summary>
...
namespace DarlLanguage
{
    ...
    public class DarlIdentifier : IdentifierTerminal
    {
        ...
        public DarlIdentifier(string name) : base(name)
        {
            QualifiedName = name;
        }
        ...
        {
            string rulesetname = "#currentruleset";
            ...
            try
            {
                ...
                if (declarationTokens.Contains(context.PreviousToken.Text)) // one of the declarations, and not yet referenced
                {
                    ...
                    {
                        if (!context.Values.ContainsKey(rulesetname))
                            context.Values.Add(rulesetname, name); //to be used thereafter for embedded
                        else
                        {
                            if ((string)context.Values[rulesetname] == name)
                                context.AddParserError("duplicate ruleset name");
                            context.Values[rulesetname] = name; //new ruleset
                        }
                    }
                ...
                else if (context.PreviousToken.Text == "{" || (context.PreviousToken.Text == "," && context.OpenBraces.Count > 1 && context.OpenBraces.First().ValueString == "{")) //inside a declaration of sets/categories
                {//the following relies on change to Darl from standard making ParserStack public.
                    if (!context.Values.ContainsKey(rulesetname))
                    {
                        context.AddParserError("set or category definitions outside of ruleset");
                        return;
                    }
                    string rname = (string)context.Values[rulesetname];
                    for (int n = context.ParserStack.Count - 1; n >= 0; n--)
                    {
                        if (context.ParserStack[n].Token != null && context.ParserStack[n].Token.Terminal != null)
                        {
                            ...
                            {//store category as ioname.categoryname
                                context.Values.Add(rname + "." + context.ParserStack[n].Token.Text + "." + context.CurrentToken.Text, context.CurrentToken.Terminal);
                                break;
                            }
                ...
                else if (context.CurrentTerminals.Count > 1)//identifier usage within a rule or wire definition and there is conflict
                {
                    ...
                    for (int n = context.ParserStack.Count - 1; n >= 0; n--)
                    {
                        if (context.ParserStack[n].Token != null && context.ParserStack[n].Token.Terminal != null)
                        {
                            ...
                            {//disambiguate 
                                ...
                                {
                                    if (context.Values.ContainsKey(context.CurrentToken.Text))
                                    {
                                        if (((Terminal)context.Values[context.CurrentToken.Text]).Equals(term))
                                        {
                                            termMatch = term;
                                            break;
                                        }
                    ...
                    string globalIOName = rname + "." + context.CurrentToken.Text;
                    ...
                    {//this is rulsetname.ioname.setname or rulsetname.ioname.catname
                        QualifiedName = rname + "." + localIO + "." + context.CurrentToken.Text;
