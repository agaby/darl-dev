/// <summary>
/// DarlResult.cs - Core module for the Darl.dev project.
/// </summary>
    ...
    public class DarlResult : IComparable
    {
        ...
        internal DarlResult Minus()
        {
            if (unknown)
                return new DarlResult(-1.0, true);
            if (!numeric)
                throw new RuleException("passing non numeric parameter(s) to minus operator");
                ...
                switch (dataType)
                {
                    ...
                    case DataType.temporal:
                        if (obj is string)
                        {
                            ...
                            values.Add((double)(dt - yearZero).TotalSeconds);
                            this.Normalise(false);
                        }
                        ...
                        {
                            List<DateTime> vals = obj as List<DateTime>;
                            vals.Sort();
                            for (int n = 0; n < vals.Count; n++)
                                values.Add((double)(vals[n] - yearZero).TotalSeconds);
                            this.Normalise(false);
                        }
        ...
        public static DarlResult operator >(DarlResult res1, DarlResult res2)
        {
            ...
            if (!res1.numeric || !res2.numeric)
                throw new RuleException("passing non numeric parameter(s) to > operator");
        ...
        public static DarlResult operator /(DarlResult res1, int res2)
        {
            ...
            if (!res1.numeric)
                throw new RuleException("Passing non numeric parameter(s) to a / operator");
            if (res2 == 0)
                throw new RuleException("Divide by zero in an integer / operator");
            DarlResult res = new DarlResult
            {
                cuts = new Interval[cutCount]
            };
        ...
        public static DarlResult operator |(DarlResult res1, DarlResult res2)
        {
            if (res1.unknown && res2.unknown)
                return new DarlResult(-1.0, true);
            if (!res1.numeric || !res2.numeric)
                throw new RuleException("passing non logical parameter(s) to a | operator");
            if ((double)res1.values[0] > 1.0 || (double)res1.values[0] < -1.0 || (double)res2.values[0] > 1.0 || (double)res2.values[0] < -1.0)
                throw new RuleException("passing non logical (out of range) parameter(s) to a | operator");
        ...
        public override bool Equals(object o)
        {
            ...
            else if (dataType == DataType.sequence && v.dataType == DataType.sequence)//sequence match
            {
                var r = new Sequence<string>();
                return r.Find(v.sequence, sequence, new SequenceComparer());
            }
            else if (dataType == DataType.textual)
            {
            ...
            switch (toType)
            {
                ...
                case DataType.temporal:
                    switch (dataType)
                    {
                        ...
                        case DataType.textual:
                            {
                                if (DateTime.TryParse(stringConstant, out DateTime val))
                                {
        ...
        {
            return yearZero + TimeSpan.FromSeconds((double)this.Value);
        }
