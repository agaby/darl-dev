        ...
        private async Task<SingleLacuna[]> FindClusters(double[,] results, double[] costs, KDTree tree)
        {
            ...
            foreach (List<int> cluster in clusters)
            {
                ...
                for (int i = 0; i < inputList.Count; i++)
                {
                    ...
                    InputInterface.DataType inputType = inputList[i].type;
                    if (inputType == InputInterface.DataType.categorical || inputType == InputInterface.DataType.presence)
                    {
                        sensitivity = 1.0 / inputList[i].categoryCount;
                    }
                    double interval = inputList[i].upperBound - inputList[i].lowerBound;
                    double lowerGap = minbound[i] - inputList[i].lowerBound;
                    double upperGap = inputList[i].upperBound - maxbound[i];
                    if (upperGap / interval > sensitivity || lowerGap / interval > sensitivity)
                    {
                        ...
                        {
                            string minCat = inputList[i].ConvertCategory(minbound[i]);
                            string maxCat = inputList[i].ConvertCategory(maxbound[i]);
                            if (minCat != maxCat)
                                throw new Exception("Cluster straddles two categories!");
                            lac.category = minCat;
                        }
                        lac.name = inputList[i].name;
                        lac.inputType = Enum.GetName(typeof(InputInterface.DataType), inputList[i].type);
                ...
                {
                    result[clusterCount].inputs[i] = filteredInputs[i];
                }
                clusterCount++;
            }
            return result;
