...
//                        }
	...
	async function  get_graph_info(){
		...
		var message = "";
		await send_to_server(gremlin_query, 'graphInfo', null, message);
	}
	...
	async function search_query() {
		...
		var message = "";
		await send_to_server(gremlin_query,'search',null,message);
	}
	...
	async function click_query(d) {
		...
		await send_to_server(gremlin_query,'click',d.id,message);
	}

	async function send_to_server(gremlin_query,query_type,active_node,message, callback){
		await run_ajax_request(gremlin_query,query_type,active_node,message,callback);				
	}

	/////////////////////////////////////////////////////////////////////////////////////////////////
	// AJAX request for the REST API
	////////////////////////////////////////////////////////////////////////////////////////////////
	async function run_ajax_request(gremlin_query,query_type,active_node,message, callback){
		// while busy, show we're doing something in the messageArea.
		$('#messageArea').html('<h3>(loading)</h3>');
		...
		*/
		try {
			var r = await gremlinFetch({ query: gremlin_query });
			if (callback) {
				callback(r);
			} else {
				handle_server_answer(r, query_type, active_node, message);
				}
		}
		catch (err) {
			alert(err[0].message);
