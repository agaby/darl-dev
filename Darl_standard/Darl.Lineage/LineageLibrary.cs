...
namespace Darl.Lineage
{
    public static class LineageLibrary
    {
        ...
        static LineageLibrary()
        {
            ...
            using (var reader = new CsvFileReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("Darl_standard.Darl.Lineage.definitions.csv")))
            {
                int index = 0;
            ...
            using (var reader = new CsvFileReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("Darl_standard.Darl.Lineage.words.csv")))
            {
                int index = 0;
                while (reader.ReadRow(columns))
                {
            ...
            using (var reader = new CsvFileReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("Darl_standard.Darl.Lineage.filteredAssociations.csv")))
            {
                int index = 0;
                while (reader.ReadRow(columns))
                {
            ...
            symSpell.LoadDictionary(Assembly.GetExecutingAssembly().GetManifestResourceStream("Darl_standard.Darl.Lineage.frequency_dictionary_en_82_765.txt"), 0, 1);
        }
